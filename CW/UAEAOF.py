...
class CWUAEAOF:
    ...
    def __init__(self, model, ae_model, adv_func, dist_func, attack_lr=1e-2,
                 binary_step=2, num_iter=200, GAMMA=0.25, low_pass=100, clip_func=None):
    ...
    def attack(self, data, target):
        ...
        for binary_step in range(self.binary_step):
            ...
            for iteration in range(self.num_iter):
                ...
                adv_loss = (1 - 2 * self.GAMMA) * self.adv_func(logits, target).mean()
                opt.zero_grad()
                adv_loss.backward()
                ...
                success_num = ((pred != target)*(lfc_pred != target)*(ae_pred != target)).sum().item()
                ...
                # update
                for e, (dist, pred, lfc_pred, ae_pred, label, ii) in \
                        enumerate(zip(dist_val, pred_val, lfc_pred_val, ae_pred_val, label_val, input_val)):
                    if dist < o_bestdist[e] and pred != label and lfc_pred != label and ae_pred != label:
                        o_bestdist[e] = dist
                        o_bestscore[e] = pred
                        o_bestattack[e] = ii
