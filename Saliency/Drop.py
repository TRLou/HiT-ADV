...
class SaliencyDrop:
    ...
    def attack(self, data, target):
        ...
        for i in range(num_rounds):
            ...
            with torch.no_grad():
                ...
                saliency = -1. * (r ** (self.alpha)) * torch.sum((data - center[:, :, None]) * grad,
                                                                     dim=1)  # [B, K]
    ...
    def sat_forward(self, data, target, budget):
        """Attack on given data to target.
        ...
        with torch.no_grad():
            ...
            # drop points with highest saliency scores w.r.t. gt labels
            # note that this is for untarget attack!
            _, idx = (-saliency).topk(k=K - self.num_drop, dim=-1)  # [B, K - k]
            _, idx_pert = (saliency).topk(k=self.num_drop, dim=-1)
            # 这里算出topk点的idx，接下来找到对应的梯度，做FGM攻击
            del_pc = torch.stack([
                data[j, :, idx[j]] for j in range(B)
            ], dim=0)  # [B, 3, K - k]
            pert_pc = data.clone().detach() + grad.sign() * budget
            # adv_pc_pert topk显著点的扰动点云 [24, 3, 1024]
            adv_pc_pert = torch.stack([
                pert_pc[j, :, idx_pert[j]] for j in range(B)
            ], dim=0)
            adv_pc = torch.cat([adv_pc_pert, del_pc], 2)

            # adv_pc = data.clone().detach()
            # for j in range(B):
            #     for i in idx_pert[j]:
            #         adv_pc[j, :, i] = adv_pc[j, :, i] + grad[j, :, i].sign() * budget

        # 返回topk点扰动的对抗点云，和删除topk点的点云
        return adv_pc.transpose(1, 2).contiguous().detach(), \
               del_pc.transpose(1, 2).contiguous().detach()
